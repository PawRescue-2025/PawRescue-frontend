...
const ShelterPage: React.FC = () => {
    ...
    const fetchShelterData = async () => {
        ...
        if (data?.ownerId) {
            const owner = await userVM.getUserById(data.ownerId);
        ...
        if (data?.id) {
            ...
            const animalsWithContacts = list.map((animal: any) => ({
                ...animal,
                shelterContacts: {
                    ownerName: ownerName ?? "",
                    ...
                    link: data.contactLink ?? null
                }
            }));
            setAnimals(animalsWithContacts);
        }
    };
    ...
    return (
        ...
        >
            ...
            {/* ----------- SHELTER CARD (ALWAYS RENDERS) ----------- */}
            <div className="glass-card">

                {/* Якщо shelter ще не завантажений — показуємо loading всередині картки */}
                {!shelter ? (
...
const ShelterPage: React.FC = () => {
    ...
    const fetchShelterData = async () => {
        ...
        if (data?.id) {
            ...
            const animalsWithContacts = list.map((animal: { [key: string]: any }) => ({
                ...animal,
                shelterContacts: {
                    ownerName: ownerName ?? "",
                    location: data.location ?? "",
