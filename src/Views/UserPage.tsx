        ...
        const postsWithAuthors = await Promise.all(
            postsData.map(async (post: any) => {
                try {
                    ...
                    return {
                        ...post,
                        author: { fullName: author.fullName, profileImage: author.photo || null },
                    };
                } catch {
                    return {
                        ...post,
                        author: { fullName: "Невідомий користувач", profileImage: null },
                    };
                }
            })
        );
    ...
    return (
        ...
        >
            <style>{`
                ...
                .side-panel { position: absolute; right: 2rem; top: 2rem; display: flex; flex-direction: column; gap: 1rem; }
                .btn-gradient { border-radius: 12px; padding: 0.6rem 1rem; background: rgba(12,95,61,0.7); color: white; border: none; font-weight: 600; transition: 0.3s ease; }
                .btn-gradient:hover { transform: translateY(-2px); box-shadow: 0 12px 25px rgba(45,134,89,0.5); background: linear-gradient(135deg, #3fb573 0%, #52d98d 50%, #6effa7 100%); }
                .profile-header { display: flex; align-items: flex-start; gap: 2rem; margin-bottom: 2rem; }
                .profile-info { display: flex; flex-direction: column; gap: 0.4rem; font-size: 0.95rem; }
                .badge { display: inline-block; padding: 0.2rem 0.5rem; border-radius: 12px; font-size: 0.8rem; font-weight: 500; margin-left: 0.5rem; color: white; }
            ...
            <div className="glass-card">
                {!user ? (
                    <p className="text-center py-5">Завантаження...</p>
                ) : (
                    <>
                        ...
                        {isOwner && (
                            <button className="btn btn-gradient w-10" onClick={() => setShowNewPostForm(prev => !prev)}>
